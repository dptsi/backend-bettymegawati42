...
func (app *application) bookCreateProcess(w http.ResponseWriter, r *http.Request) {
	...
	req, _ := http.NewRequest("POST", app.readinglist.Endpoint, bytes.NewBuffer(data)) //bikin post untuk reaching readinglist endpoint
	req.Header.Set("Content-Type", "application/json")

	client := &http.Client{}
	resp, err := client.Do(req) //bikin client yang kirim web request ke web service
	if err != nil {
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
	...
	http.Redirect(w, r, "/", http.StatusSeeOther) //redirect ke homepage
}
